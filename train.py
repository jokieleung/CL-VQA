...
def train(model, train_loader, eval_loader,args,qid2type):
    ...
    elif mode=='v_debias_contras' or mode=='v_debias_ori_contras_cos' or mode == 'v_debias_ori_contras_eucl':
        topv=args.topv
        top_hint=args.top_hint
    elif mode=='q_v_debias' or mode=='q_v_debias_ori_contras_cos' or mode=='q_v_debias_inner_contras':
        topv=args.topv
        top_hint=args.top_hint
        topq=args.topq
        keep_qtype=args.keep_qtype
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b, hintscore,type_mask,notype_mask,q_mask) in tqdm(enumerate(train_loader), ncols=100,
            ...
            if mode=='updn':
                ...
                pred, _, _ = model(v, q, None, b, v_mask) # P = VQA(I+,Q)
                ...
                pred, loss, _ = model(v, q, a2, b, v_mask) # P = VQA(I-,Q)
            ...
            elif mode=='v_debias_contras':
                ## first train
                pred, loss, _, feature_ori = model(v, q, a, b, None)
                visual_grad=torch.autograd.grad((pred * (a > 0).float()).sum(), v, create_graph=True)[0]
                ...
                pred, _, _, feature_pos = model(v, q, None, b, v_mask) # P = VQA(I+,Q)
                ...
                pred, loss, _, feature_neg = model(v, q, a2, b, v_mask) # P = VQA(I-,Q)

                #Compute the contras loss
                # inner product
                # pos = (feature_ori * feature_pos).sum(1)
                # neg = (feature_ori * feature_neg).sum(1)
                # cosine_similarity
                pos = torch.cosine_similarity(feature_ori, feature_pos, dim=1)
                neg = torch.cosine_similarity(feature_ori, feature_neg, dim=1)
            ...
            elif mode=='v_debias_ori_contras_cos' or mode=='v_debias_ori_contras_eucl' or mode == 'q_v_debias_ori_contras_cos':
                if not mode == 'q_v_debias_ori_contras_cos':
                    ## first train
                    pred, loss, _, feature_ori = model(v, q, a, b, None)
                    visual_grad=torch.autograd.grad((pred * (a > 0).float()).sum(), v, create_graph=True)[0]
                    ...
                    # cosine_similarity
                    if mode=='v_debias_ori_contras_cos':
                        pos = torch.cosine_similarity(feature_ori, feature_pos, dim=1)
                        neg = torch.cosine_similarity(feature_ori, feature_neg, dim=1)
                        pos_dis = 1 - pos
                        neg_dis = 1 - neg
                        contras_loss_pos = pos_dis
                        margin = args.margin
                        contras_loss_neg = torch.clamp(margin - neg_dis, min=0.0)
                    # euclidean_distance
                    elif mode=='v_debias_ori_contras_eucl':
                        pos_dis = nn.functional.pairwise_distance(feature_ori, feature_pos, p=2)
                        neg_dis = nn.functional.pairwise_distance(feature_ori, feature_neg, p=2)
                        contras_loss_pos = torch.pow(pos_dis, 2)
                        margin = args.margin
                        contras_loss_neg = torch.pow(torch.clamp(margin - neg_dis, min=0.0), 2)
                ...
                elif mode=='q_v_debias_ori_contras_cos':
                    ...
                    if random_num<=qvp:
                        ## first train
                        pred, loss, word_emb, feature_ori = model(v, q, a, b, None)
                        word_grad = torch.autograd.grad((pred * (a > 0).float()).sum(), word_emb, create_graph=True)[0]
                        ...
                        pred, _, _, feature_pos = model(v, q2, None, b, None)
                        ...
                        pred, loss, _, feature_neg = model(v, q3, a2, b, None)
                        ...
                        loss += args.contras_loss_weight * contras_loss # with CSS
                        # loss = contras_loss # only_contras_loss
                        ...
                        pred, _, _, feature_pos = model(v, q, None, b, v_mask)
                        pred_ind = torch.argsort(pred, 1, descending=True)[:, :5]
                        false_ans = torch.ones(pred.shape[0], pred.shape[1]).cuda()
                        false_ans.scatter_(1, pred_ind, 0)
                        a2 = a * false_ans
                        ...
                        pred, loss, _, feature_neg = model(v, q, a2, b, v_mask)
            ...
            elif mode=='q_v_debias_inner_contras':
                random_num = random.randint(1, 10)
                if keep_qtype == True:
                    sen_mask = type_mask
                else:
                ...
                if random_num<=qvp:
                    ...
                    pred_pos, _, _, feature_pos = model(v, q2, None, b, None) # P = VQA(I,Q+, )
                    pred_ind = torch.argsort(pred_pos, 1, descending=True)[:, :5]
                    false_ans = torch.ones(pred_pos.shape[0], pred_pos.shape[1]).cuda()
                    false_ans.scatter_(1, pred_ind, 0)
                    a2 = a * false_ans
                    q3 = copy.deepcopy(q)
                    if dataset=='cpv1':
                    ...
                    pred_neg, loss_neg, _, feature_neg = model(v, q3, a2, b, None) # P = VQA(I,Q-, )
                    ...
                    loss = loss_neg + args.contras_loss_weight * contras_loss
                    ...
                    pred_pos, _, _, feature_pos = model(v, q, None, b, v_mask) # P = VQA(I+,Q)
                    v_mask_pos = v_mask
                    ...
                    v_mask_neg = 1 - v_mask_pos

                    pred_neg, loss_neg, _, feature_neg = model(v, q, a2, b, v_mask_neg) # P = VQA(I-,Q)
